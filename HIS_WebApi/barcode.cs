        ...
        public async Task<string> Decode([FromForm] IFormFile file)
        {
            try
            {
                ...
                if (file == null || file.Length == 0)
                {
                    ...
                    new {
                        type = "error",
                        message = "未收到圖片（需使用 multipart/form-data 上傳）"
                    }
                }
                    });
                }
                ...
                // 條碼/QRCode 解碼
                var list = BarcodeDecoder.Decode(bmp);

                if (list == null || list.Count == 0)
                {
                    return JsonSerializer.Serialize(new
                    {
                        results = Array.Empty<object>()   // 找不到條碼 → 空陣列
                    });
                }
                ...
                // 判斷是否為 QR CODE
                string detectedType;
                string formatUpper = item.Format?.ToUpper() ?? "";

                if (formatUpper.Contains("QR"))
                    detectedType = "qrcode";
                else
